...
public class DeliveryControllerImpl implements DeliveryController {
    ...
    public Delivery crearDelivery(@RequestBody NewDelivery newPedidoDelivery,
                              @PathVariable("restaurantId") Long restaurantId,
                              @RequestParam(name = "productos", required = false) List<Integer> productList,
                              Authentication auth){
        return service.crearDelivery(newPedidoDelivery,productList, restaurantId, Long.parseLong(auth.getName()));
    }
    ...
    public void addProducto(@PathVariable("restaurantId") long restaurantId,
                            ...
                            Authentication auth) {
        service.addProducto(restaurantId, deliveryId, productList, Long.parseLong(auth.getName()));
    }
    ...
    public void aplicarDescuento(@PathVariable("restaurantId") long restaurantId,
                                 ...
                                 Authentication auth){
        service.aplicarDescuento(restaurantId, deliveryId, porcentajeDescuento, Long.parseLong(auth.getName()));
    }
    ...
    public Factura liberarPedido(@PathVariable("restaurantId") long restaurantId,
                                 @PathVariable("deliveryId") long deliveryId,
                                 Authentication auth) {
        return service.liberarPedido(restaurantId, deliveryId, Long.parseLong(auth.getName()));
    }
