...
public class MesaControllerImpl implements MesaController {
	...
	public void createPedido(@RequestBody NewPedido request, @PathVariable("mesaId") long mesaId,
							 @PathVariable("restaurantId") long restaurantId, Authentication auth) {		
		service.createPedido(request, mesaId, restaurantId, Long.parseLong(auth.getName()));
	}
	...
	public Pedido getPedidoActivo(@PathVariable(value = "restaurantId") Long restaurantId,
								  @PathVariable("mesaId") long mesaId, Authentication auth) {
		return service.mostrarPedidoActivo(restaurantId, mesaId, Long.parseLong(auth.getName()));
	}
	...
	public List<Producto> getProductos(@PathVariable("pedidoId") long pedidoId,
									@PathVariable("restaurantId") long restaurantId, Authentication auth) {
		return pedidoService.getProductos(pedidoId, restaurantId, Long.parseLong(auth.getName()));
	}
