...
public class RestaurantControllerImpl implements RestaurantController {
	...
	@Secured({ "ADMIN" })
	@PostMapping
	public Restaurant createRestaurant(@RequestBody NewRestaurant request) {
	...
	public Mesa addMesa(@PathVariable("restaurantId") long restaurantId,
						@RequestBody NewMesa request, Authentication auth) {
		return service.addMesa(request, restaurantId, Long.parseLong(auth.getName()));
	}
	
	@Secured({ "OPERADOR" })
	@GetMapping(path = "/{restaurantId}/mesas")
	public List<Mesa> getMesas(@PathVariable("restaurantId") long restaurantId,
							   Authentication auth) {
	...
	public void updateMesa(@PathVariable("mesaId") long mesaId,
						   @RequestBody NewMesa request,
						   @PathVariable("restaurantId") long restaurantId,
						   Authentication auth) {
		mesaService.updateMesa(mesaId, request, restaurantId,
							   Long.parseLong(auth.getName()));
	}
	
	@Secured({ "OPERADOR" })
	@DeleteMapping(path = "/{restaurantId}/mesas/{mesaId}")
	public void deleteMesa(@PathVariable("mesaId") long mesaId,
						   @PathVariable("restaurantId") long restaurantId,
						   Authentication auth) {
